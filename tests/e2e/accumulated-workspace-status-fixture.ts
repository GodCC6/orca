...
const TRAFFIC_CONTROLLER_KEY = '__orcaAccumulatedStatusTrafficController'
...
export async function startAccumulatedStatusTraffic(
  ...
  await electronApp.evaluate(
    ({ BrowserWindow }, { controllerKey, intervalMs, panes }) => {
      const appWindow = BrowserWindow.getAllWindows().find(
        (candidate) => !candidate.isDestroyed() && !candidate.webContents.isDestroyed()
      )
      if (!appWindow) {
      ...
      if (Reflect.has(globalThis, controllerKey)) {
      ...
      Reflect.set(globalThis, controllerKey, {
        stop: () => {
          clearInterval(timer)
    ...
    { controllerKey: TRAFFIC_CONTROLLER_KEY, intervalMs, panes }
  )
...
export async function stopAccumulatedStatusTraffic(
  ...
  const mainStats = await electronApp.evaluate(
    (
      _electron,
      controllerKey
    ): {
      ...
      const candidate: unknown = Reflect.get(globalThis, controllerKey)
      if (
        !candidate ||
